Zm.managements.check_guest_order= { 
    init: function() { 
        ...
        var store = new Ext.data.JsonStore({ 
            url: '/kmanagements/get_guest_order.json',
            fields: ['colors','price', 'client','contract','total_price','quality','shipment','payment','lading_bill','production_date','remark'],
            root: 'guest_order',
            autoLoad: true
        });
                ...
                handler: function(){ clientorderenquirydetail.show(); }
               	...
               	handler: function(){
                    Ext.Ajax.request({
                        ...
                        success: function(){ Ext.Msg.alert('Good','It is OK now!'); },
                        failure: function(){ Ext.Msg.alert('Oh no','It is so bad!'); }
                    });
               	}
  	    		},{				
  	    		    text: '打开提单',
